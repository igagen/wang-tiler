var WangView = Backbone.View.extend({
  ...
  initialize: function() {
    ...
    for (var i = 0; i < this.TILES.length; i++) {
      ...
      this.tiles[tile] = {};
      this.tiles[tile].canvas = this[tile + 'Canvas'] = $("#wang-tiles ." + tile);
      this.tiles[tile].context = this[tile + 'Context'] = this[tile + 'Canvas'][0].getContext("2d");
    ...
    $("#wang-tiles canvas, #sub-samples canvas, #scratch").each(function() {
  ...
  handleMouseUp: function(event) {
    this.sampleRect = this.getSelectionRect(event);
  ...
  handleMouseMove: function(event) {
    if (this.sampling) {
      var rect = this.getSelectionRect(event);
